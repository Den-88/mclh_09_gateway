...
public:
  ...
  void set_update_interval(uint32_t update_interval) {
    ESP_LOGD("mclh-09", "Setting update interval to %dms", update_interval);
    for (size_t i = 0; i < device_count; i++)
      ble_client[i]->set_update_interval(update_interval);
  }
...
private:
  ...
  float interpolate(float value, std::vector<float> &input, std::vector<float> &output, bool limit = false) {
    if (input.size() != output.size() || input.size() < 3) {
      ESP_LOGE("mclh-09", "Bad size of input/output arrays for interpolate()");
      return NAN;
    }
    size_t i = 0;
    ...
    ESP_LOGV("mclh-09", "interpolate(%f) = %f, index = %d, (%f, %f)", value, result, i, input[i], input[i + 1]);
